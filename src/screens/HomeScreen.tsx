...
// =========================================================================
// CONSTANTES DE CARGA PROGRESIVA
// =========================================================================
const INITIAL_BLOCKS = 1; // Mostrar 4 bloques de galería al inicio
const LOAD_MORE_BLOCKS = 1; // Cargar 2 bloques adicionales por vez
const MAX_BLOCKS_BEFORE_COMPRESS = 3; // El número de bloques que se muestran antes de mostrar "Ver Menos"
...
function buildGalleryBlocks(images: GalleryImage[]) {
  ...
  while (i < images.length) {
    // evita repetir layout seguido (se ve más pro)
...
// =========================================================================
// GALLERY BLOCK COMPONENT (NUEVOS PATRONES)
// =========================================================================
function GalleryBlock({
  ...
  // 1) 3 en fila (todas flex:1 => no hay huecos)
  if (layout === "row3") {
    return (
      <View style={g.row}>
  ...
  // 2) 2 columnas: izquierda grande, derecha 2 apiladas (sin huecos)
  if (layout === "hero2") {
    return (
      <View style={g.row}>
  ...
  // 3) Wide (una sola, pero controlada para que no se repita “banner” demasiado)
  if (layout === "wide") {
    return (
      <View style={g.block}>
  ...
  // 4) Mosaico 2x2 (compacto, sin huecos)
  if (layout === "mosaic2x2") {
    return (
      <View style={g.block}>
        <View style={g.row}>
...
export function HomeScreen() {
  ...
  // =========================================================================
  // ESTADOS PARA CARGA PROGRESIVA DE GALERÍA
  // =========================================================================
  ...
  // Calcular todos los bloques de galería disponibles
  const galleryBlocks = useMemo(
    () => buildGalleryBlocks(gallery),
    [gallery]
  );
  
  // Bloques que realmente se muestran
  const visibleGalleryBlocks = useMemo(() => {
    return galleryBlocks.slice(0, visibleBlockCount);
  }, [galleryBlocks, visibleBlockCount]);

  // Checar si hay más bloques para cargar
  const hasMoreBlocks = galleryBlocks.length > visibleBlockCount;
  ...
  // =========================================================================
  // MANEJADOR DE "VER MÁS / VER MENOS"
  // =========================================================================
  const handleToggleGallery = () => {
    if (visibleBlockCount >= MAX_BLOCKS_BEFORE_COMPRESS) {
      // Regresar al estado inicial con scroll suave
      setVisibleBlockCount(INITIAL_BLOCKS);
    } else {
      // Cargar más bloques con un delay para simular carga
  ...
  return (
    ...
    >
      ...
      {/* ===== GALERÍA (IMPLEMENTACIÓN DE "VER MÁS/MENOS") ===== */}
      {gallery.length > 0 && (
        <View style={[styles.section, styles.sectionTint, styles.end]}>
          <Text style={[styles.sectionTitle, styles.centro ]}>Galería</Text>
          
          {loadingGallery ? (
            <SkeletonGallery />
          ) : (
            <View style={{ marginTop: 12 }}>
              {/* Renderizar solo los bloques visibles */}
          ...
          {/* Botón de "Ver Más / Ver Menos" centralizado con mejor diseño */}
          {galleryBlocks.length > INITIAL_BLOCKS && (
            ...
            >
              ...
              ) : visibleBlockCount >= MAX_BLOCKS_BEFORE_COMPRESS ? (
...
// STYLES
// =========================================================================
const GALLERY_GAP = 6;     // ✅ poco espacio entre imágenes (como pides)
...
const g = StyleSheet.create({
  ...
  tileShade: {
    ...
    backgroundColor: "rgba(0,0,0,0.04)", // sutil, elegante
  },

  // tamaños SIEMPRE con flex:1 en filas para evitar blancos
  tileSm: { flex: 1, height: 110 },
  tileMd: { flex: 1, height: 112 },

  // hero: toma más ancho pero sigue siendo layout estable
  tileHero: { flex: 1.35, height: 230 },

  // wide: controlado (no gigante)
...
const styles = StyleSheet.create({
  ...
  aboutHero: {
    ...
    ...Platform.select({
      ...
      android: { elevation: 4 },
      web: { boxShadow: "0 6px 16px rgba(0,0,0,0.14)" },
    }),
  },
  aboutHeroImg: {
    width: "100%",
  ...
  // Estilos de los diferentes patrones
  galleryBig: {
    width: "48%",
    height: 240,
  },
